function TechDiversityGender() {
  ...
  this.femaleColour = color(246, 109, 68);
  this.maleColour = color(100, 194, 166);
    ...
    this.drawRects = function(){
        ...
        for (var i = 0; i < this.data.getRowCount(); i++) {
          ...
          rect(this.layout.leftMargin + this.mapPercentToWidth(company.female),
            ...
            //decet mouse hover over female rect
            push();
            textSize(12)
            textAlign(LEFT)
            if(mouseX > this.layout.leftMargin && 
               ...
               // Redraw female rect with darker color
               fill(186, 49,8);               
               rect(this.layout.leftMargin,
                    lineY,
                    this.mapPercentToWidth(company.female),
                ...
                // Display information for each company when hovering over rect
                var txt = company.name+" - Female: "+company.female+"%"
                fill(40)
                rect(mouseX+60,mouseY-12,textWidth(txt)+10, lineHeight-5)
                fill(240)
                text(txt,mouseX+65,mouseY-7)           

            }// Decet mouse hover over male rect
            else if(mouseX > this.layout.leftMargin + this.mapPercentToWidth(company.female) &&
                    mouseX < this.layout.leftMargin + this.mapPercentToWidth(company.female) + this.mapPercentToWidth(company.male) &&
                    mouseY > lineY &&
                    mouseY < lineY + (lineHeight - this.layout.pad)
                   ){

                    // Redraw male rect with darker color
                    fill(40, 131, 106);               
                    rect(this.layout.leftMargin + this.mapPercentToWidth(company.female),
                            lineY,
                            this.mapPercentToWidth(company.male),
                    ...
                    // Display information for each company when hovering over rect
                    var txt = company.name+" - Male: "+company.male+"%"
                    fill(40)
                    rect(mouseX+60,mouseY-12,textWidth(txt)+10, lineHeight-5)
                    fill(240)
                    text(txt,mouseX+65,mouseY-6)           
            }
            pop();
        }
